...
+ (instancetype)shareInstance{
    static LYSystemManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[LYSystemManager alloc] init];
    });
    return manager;
}
// 返回设备型号信息
...
+ (void)ly_notifitionReachability:(void(^)(LYS_Reachability *reachability))action reachability:(LYS_Reachability *)reachability promptly:(BOOL)promptly
{
    ...
    reachability.notifitionCallBack = ^(LYS_Reachability *reachability) {
